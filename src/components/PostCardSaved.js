...
import { BsBookmark, BsBookmarkFill,BsEye,BsEmojiSmile,BsTrash, BsBoxArrowDownLeft } from 'react-icons/bs';
...
export default function PostCardSaved() {
  ...
  const fetchMoreData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://127.0.0.1:8090/api/collections/posts/records?page=2&perPage=4');
  ...
  const fetchComments = async (serial) => {
    try {
      const link='http://127.0.0.1:8090/api/collections/comments/records?filter=(postserial="'+serial+'")&sort=-created'
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.get('http://127.0.0.1:8090/api/collections/posts/records?page=1&perPage=8&filter=(savedppl~",0"||savedppl~",0,"||savedppl~"0,"||savedppl~"0")');
        const temp=response.data.items;
        for(let i=0;i<temp.length;i++){
          if(temp[i].savedppl.split(",").indexOf("0")<0){
              temp.splice(i,1);
          }
        }
        setNewPosts(temp);
...
str=str+","+"0";
    ...
    // const link='http://127.0.0.1:8090/api/collections/posts/records?filter=(serial="'+serial+'")';
  ...
  return (
    <>
      ...
      <DrawerContent bg={'rgb(33,33,33)'} color={'rgb(242,242,242)'}>
        <DrawerHeader borderBottomWidth='1px' fontSize={'sm'}>{cort ? 'Creators' : 'All tags'}</DrawerHeader>
        <DrawerBody pt={'10px'} pb={'20px'}>
        {cort ? newposts[crt]!=undefined ? newposts[crt].creators.split(',').map((abc) => (
  <Link href={'/'+abc}>
...
<TagLabel>{abc}</TagLabel>
      ...
      <></>
      :
     newposts[crt]!=undefined ? newposts[crt].tags.split(',').map((abc) => (
      <Link href={'/'+abc}>
...
<AlertDialog
    ...
    >
      ...
      <DrawerContent bg={'rgb(33,33,33)'} color={'rgb(242,242,242)'}>
        ...
        <DrawerHeader textAlign={'center'} fontSize="md">{comments.length} Comments</DrawerHeader>

        <DrawerBody>
          {comments.length==0?
            <Text fontSize={'sm'} color={'gray'} textAlign={'center'}>Be the first to comment... </Text>
            :
            <></>
          }
...
<Image className={styles.img} src={'/posts/post_'+post.serial+'.png'} width={1000} height={1000} alt='post' />                    
                  </div>
                  {/* <div className={styles.overlay}>
                      <button><Link target='_black' href={post.link}>Visit Site&nbsp;<Icon as={BiLinkExternal}/></Link></button>
                  </div> */}
                  ...
                  {post.tags.length>0 ? post.tags.split(',').map((abc) => (
            <Tag size='sm' mr='5px'  variant={'outline'} colorScheme='purple'>
          <TagLabel>{abc}</TagLabel>
        </Tag>
                  ))
                  :
...
<Button onClick={()=>{setCorT(true); setCrt(index); openCrt();}} as={Button} display={'flex'} alignItems={'center'} variant={'outline'} size={'xs'} colorScheme='teal'>
<Icon as={BsFillPeopleFill} />&nbsp;Creators
</Button>
{/* {post.creators.split(',').map((abc) => (
  <Link href={'/'+abc} >
  <MenuItem>
  <Tag size='md' width={"100%"} colorScheme='white'>
<Avatar src={'/avatars/avatar_'+abc+'.png'} size='xs' name={abc} ml={-3} mr={2} />
<TagLabel>{abc}</TagLabel>
</Tag>
  </MenuItem>
  </Link>
...
<Button  ml={'auto'} display={'flex'} alignItems={'center'} colorScheme='teal' size={'xs'} variant={'outline'}>visit site</Button>
              ...
              {/* <p style={{"fontSize":"13px","color":"gray","marginLeft":"auto"}} >{post.created.substring(0,10)}</p> */}
                  </>
                  }
             ...
             {loading ? (
              <>
               {arr.map((x)=>(
      <div className={styles.postcard} style={{"border":"none","borderRadius":"0px"}}>
        <Stack>
